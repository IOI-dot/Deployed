...
function Search() {
  ...
  const [editForm, setEditForm] = useState({ roomId: "", startTime: "", endTime: "", date: "" });

  const [searchText, setSearchText] = useState("");
  const [capacity, setCapacity] = useState(4);
  const [role, setRole] = useState("Student");
  const [filters, setFilters] = useState({ display4k: false, whiteboards: false, projector: false, videoConf: false });
  ...
  useEffect(() => {
    if (activeTab === "Search") { fetchRooms(); fetchQuota(); }
  }, [activeTab]);

  useEffect(() => {
    if (activeTab === "My Bookings") { fetchMyBookings(); fetchRooms(); }
  }, [activeTab]);

  // --- Cancel handler (uses YOUR soft-cancel endpoint) ---
  ...
  // --- Edit handler (uses new /edit endpoint with row-level locking) ---
  ...
  // --- Timeline helpers for edit modal ---
  const getEditStartHour = () => editForm.startTime ? parseInt(editForm.startTime.split(":")[0], 10) : null;
  const getEditEndHour = () => editForm.endTime ? parseInt(editForm.endTime.split(":")[0], 10) : null;
  ...
  const isEditSlotBooked = (hr) => {
    ...
    return !!selectedEditRoom.bookings?.find((b) => {
      const start = parseInt(b.start_hour, 10);
      const end = b.end_hour ? parseInt(b.end_hour, 10) : start + 1;
      if (editModal && b.booking_id === editModal.booking_id) return false;
      return hr >= start && hr < end;
    });
  };

  const handleEditSlotClick = (hr) => {
    if (!editForm.roomId) { alert("Please select a room first."); return; }
    if (isEditSlotBooked(hr)) return;
    const currentStart = getEditStartHour();
    if (currentStart === null || hr < currentStart) {
      setEditForm({ ...editForm, startTime: `${hr.toString().padStart(2, "0")}:00`, endTime: `${(hr + 1).toString().padStart(2, "0")}:00` });
      return;
    }
    const newEnd = hr + 1;
    if (newEnd - currentStart > 4) { alert("Cannot book more than 4 hours."); return; }
    for (let i = currentStart; i < newEnd; i++) {
      if (isEditSlotBooked(i)) { alert("Selection contains booked slots."); return; }
    }
    setEditForm({ ...editForm, endTime: `${newEnd.toString().padStart(2, "0")}:00` });
  };
  ...
  const filteredRooms = useMemo(() => {
    return rooms.filter((room) => {
      const matchesSearch = room.room_name.toLowerCase().includes(searchText.toLowerCase());
      const matchesCapacity = room.capacity >= Number(capacity);
      const matchesFeatures = activeFeatures.length === 0 ||
        activeFeatures.every((feature) => (room.technology || "").toLowerCase().includes(feature.toLowerCase().split(" ")[0]));
      return matchesSearch && matchesCapacity && matchesFeatures;
    });
  }, [rooms, searchText, capacity, activeFeatures]);

  const suggestions = rooms.filter((room) => room.capacity >= Number(capacity) && !filteredRooms.some((r) => r.id === room.id));
        ...
        {activeTab === "Search" && (
          <>
            <div className="search-top-bar">
              ...
              <input type="text" placeholder="Quick search..." value={searchText} onChange={(e) => setSearchText(e.target.value)} className="search-quick-input" />
            </div>
              ...
              <section className="search-filters-panel">
                <div className="search-card">
                  ...
                  <div className="search-capacity-row">
                    <input type="number" min="1" value={capacity} onChange={(e) => setCapacity(e.target.value)} className="search-capacity-input" />
                    <span>People</span>
                  </div>
                  <label className="search-filter-label">Your Role</label>
                  <div className="search-role-buttons">
                    <button type="button" className={role === "Student" ? "search-role-btn active-role" : "search-role-btn"} onClick={() => setRole("Student")}>Student</button>
                    <button type="button" className={role === "TA / Faculty" ? "search-role-btn active-role" : "search-role-btn"} onClick={() => setRole("TA / Faculty")}>TA / Faculty</button>
                  </div>
                  ...
                  <button type="button" className="search-reset-btn" onClick={() => setFilters({ display4k: false, whiteboards: false, projector: false, videoConf: false })}>Reset All Filters</button>
                </div>
                <div className="search-card search-quota-card">
                  ...
                  <div className="search-progress-bar" style={{ backgroundColor: "#e2e8f0", overflow: "hidden", borderRadius: "8px" }}>
                    <div className="search-progress-fill" style={{ width: `${Math.min((usedHours / dailyQuota) * 100, 100)}%`, backgroundColor: usedHours >= dailyQuota ? "#dc2626" : "#2563eb", height: "100%", transition: "all 0.5s ease" }} />
                  </div>
                </div>
              </section>
              ...
              <section className="search-results-panel">
                <div className="search-results-header">
                  ...
                  <div className="search-active-tags">
                    {activeFeatures.map((f, i) => <span key={i} className="search-filter-tag">{f}</span>)}
                  </div>
                </div>
                  ...
                  {loading ? <p>Loading Database...</p> : filteredRooms.length > 0 ? (
                    filteredRooms.map((room) => (
                      <div className="search-room-card" key={room.id}>
                        <div className="search-room-top"><span className="search-status-badge">Available Now</span></div>
                        <h3>{room.room_name}</h3>
                        <p className="search-room-location">AUC Campus · {room.technology}</p>
                        <div className="search-room-features"><span>Cap: {room.capacity}</span></div>
                        <div className="search-availability-section">
                          <div className="search-availability-head">
                            <span>24-Hour Timeline</span><span>00:00 — 23:00</span>
                          </div>
                          <div className="search-timeline" style={{ height: "20px", display: "flex", gap: "1px", background: "#eee", borderRadius: "4px", overflow: "hidden" }}>
                            {hoursOfDay.map((hour) => {
                              ...
                              const isBooked = !!room.bookings?.find(b => {
                                const start = parseInt(b.start_hour, 10);
                                const end = b.end_hour ? parseInt(b.end_hour, 10) : start + 1;
                                return hr >= start && hr < end;
                              });
                              return <div key={hour} title={isBooked ? `Booked at ${hour}` : `Free at ${hour}`} style={{ flex: 1, backgroundColor: isBooked ? "#dc2626" : "#16a34a" }} />;
                            })}
                          </div>
                        </div>
                    ...
                    <div className="search-empty-state"><h3>No exact matches found</h3><p>Try changing the filters or search text.</p></div>
                  )}
                </div>
                  ...
                  <div className="search-suggested-grid">
                    {suggestions.slice(0, 2).map((room) => (
                      <div className="search-suggested-card" key={room.id}>
                        ...
                        <div><h4>{room.room_name}</h4><p>Capacity: {room.capacity}</p><div className="search-mini-tags"><span>{room.technology}</span></div></div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                return (
                  <div className={`search-room-card ${isCancelled ? "booking-cancelled" : ""}`} key={b.booking_id}>
                    <div className="search-room-top">
                      <span className="search-status-badge" style={{ backgroundColor: isCancelled ? "#94a3b8" : "#3b82f6", color: "white" }}>
                        {isCancelled ? "Cancelled" : "Confirmed"}
                      </span>
                    </div>
                    <h3>{b.room_name}</h3>
                    <p className="search-room-location">📍 AUC New Cairo Campus</p>
                    <div className="search-room-features" style={{ display: "flex", flexDirection: "column", gap: "6px", marginTop: "12px", marginBottom: "12px" }}>
                      <span>📅 <strong>Date:</strong> {b.date}</span>
                      <span>⏰ <strong>Time:</strong> {b.start_time} — {b.end_time}</span>
                      <span>👥 <strong>Cap:</strong> {b.capacity}</span>
                      <span>💻 <strong>Tech:</strong> {b.technology}</span>
                    </div>
                    ...
                    {!isCancelled && (
                      <button
                        ...
                        onClick={() => {
                          setEditModal(b);
                          setEditForm({ roomId: b.room_id || "", startTime: b.start_time || "", endTime: b.end_time || "", date: b.date || "" });
                        }}
                      >
                        Edit Booking
                      </button>
                    ...
                    {!isCancelled && (
                      <button className="cancel-booking-btn" onClick={() => setCancelModal(b)} disabled={cancellingId === b.booking_id}>
            ...
            {editModal && (
              <div className="cancel-modal-overlay" onClick={() => setEditModal(null)}>
                <div className="cancel-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "650px" }}>
                  ...
                  <label style={{ display: "block", textAlign: "left", marginBottom: "6px", fontWeight: "700" }}>Room</label>
                  <select
                    value={editForm.roomId}
                    onChange={(e) => setEditForm({ ...editForm, roomId: e.target.value, startTime: "", endTime: "" })}
                    style={{ width: "100%", marginBottom: "15px", padding: "12px", borderRadius: "8px" }}
                  >
                    <option value="">Select room</option>
                    {rooms.map((room) => (
                      <option key={room.id} value={room.id}>{room.room_name} — Cap: {room.capacity} — {room.technology}</option>
                    ))}
                  </select>

                  <label style={{ display: "block", textAlign: "left", marginBottom: "6px", fontWeight: "700" }}>Date</label>
                  <input
                    type="date"
                    value={editForm.date}
                  ...
                  <div style={{ display: "flex", gap: "2px", height: "40px", marginBottom: "10px", background: "#eee", borderRadius: "6px", overflow: "hidden" }}>
                    {Array.from({ length: 24 }, (_, hr) => {
                      ...
                      let bgColor = "#16a34a";
                      if (isBooked) bgColor = "#dc2626";
                      if (isSelected) bgColor = "#2563eb";
                      return (
                        <div key={hr} onClick={() => handleEditSlotClick(hr)} title={`${hr}:00`}
                          style={{ flex: 1, backgroundColor: bgColor, cursor: isBooked ? "not-allowed" : "pointer" }} />
                      );
                    })}
                  </div>

                  <p style={{ marginBottom: "15px", fontWeight: "700" }}>
                    Selected: {editForm.startTime && editForm.endTime ? `${editForm.startTime} — ${editForm.endTime}` : "No time selected"}
                  </p>

                  <div className="cancel-modal-actions">
                    <button className="cancel-modal-keep" onClick={() => setEditModal(null)}>Cancel</button>
            ...
            {cancelModal && (
              <div className="cancel-modal-overlay" onClick={() => setCancelModal(null)}>
                <div className="cancel-modal" onClick={(e) => e.stopPropagation()}>
                  ...
                  <p>Are you sure you want to cancel your booking for <strong>{cancelModal.room_name}</strong> on <strong>{cancelModal.date}</strong> from <strong>{cancelModal.start_time}</strong> to <strong>{cancelModal.end_time}</strong>?</p>
                  <p className="cancel-modal-sub">This will free the time slot for other users.</p>
                  <div className="cancel-modal-actions">
                    <button className="cancel-modal-keep" onClick={() => setCancelModal(null)}>Keep Booking</button>
                    <button className="cancel-modal-confirm" onClick={() => handleCancelBooking(cancelModal)} disabled={cancellingId === cancelModal.booking_id}>
